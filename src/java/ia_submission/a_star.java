...
public class a_star extends DefaultInternalAction{
    ...
    private double distance(int dx, int dy) {
        int xval = distMod(this.now.x + dx, this.xend, Handler.getInstance().getWidth());
        int yval = distMod(this.now.y + dy, this.yend, Handler.getInstance().getHeight());
        return xval + yval;
    }
    ...
    private void addOpenList() {
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                ...
                tmpNode.set(this.now, mod(this.now.x + x,Handler.getInstance().getWidth()), mod(this.now.y + y,Handler.getInstance().getHeight()), this.now.g, this.distance(x, y));
                if ((x != 0 || y != 0)
                    && this.map[mod(this.now.y + y,Handler.getInstance().getHeight())][mod(this.now.x + x,Handler.getInstance().getWidth())] != 'O'
    ...
    public List<int[]> get_route(char[][] map, int xstart, int ystart, int xgoal, int ygoal) {
        //initialise fields
        this.xstart = mod(xstart, Handler.getInstance().getWidth());
        this.ystart =  mod(ystart, Handler.getInstance().getHeight());
        this.map = map;
        this.now.x = this.xstart;
        this.now.y = this.ystart;
        
        //find the path to the goal position
        this.path = findRoute(mod(xgoal, Handler.getInstance().getWidth()), mod(ygoal, Handler.getInstance().getHeight()));
