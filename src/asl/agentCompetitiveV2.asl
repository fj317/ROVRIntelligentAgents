	...
	if (ResourceType == "Obstacle") {
		...
		if (AgentResourceType = "NA") {
			...
			if (ResourceType == "Gold") {
				.send(agentCompetitiveV1, tell, typeOfResource("Diamond"));
			} elif (ResourceType == "Diamond") {
				.send(agentCompetitiveV1, tell, typeOfResource("Gold"));
			}
		}
		...
		if (ResourceType == NewAgentResourceType) {
			...
			.send(agentCompetitiveV1, tell, resources([XPos + XDistToResource, YPos + YDistToResource, Quantity]));
		}
	}
	.
